...
coldata <- read.delim(file = "/Users/fitzsimmonscm/Documents/Projects_and_Data/Collaborations/Gottesman/RNASeq_Bergel/Gottesman_Rhomedepsin.txt",
                      header=TRUE, sep = "\t", row.names = 1)

print(coldata)
nrow(coldata)
countdata1 <- read.delim(file = "/Users/fitzsimmonscm/Documents/Projects_and_Data/Collaborations/Gottesman/RNASeq_Bergel/RawCountFile_rsemgenes.txt", row.names = 1, sep="\t")
...
topVarGenes <- head( order( rowVars( assay(rld) ), decreasing=TRUE ), 250 )
...
write.csv(res_export, file="/Users/fitzsimmonscm/Documents/Projects_and_Data/Collaborations/Gottesman/RNASeq_Bergel/2019.03.16_Rhomedepsin_RNASeq_R_V_D9vs_NR_V.csv")
```
# Comparing R_D9 and NR_D9
```{r}

# Here, we are asking to compare 2 treatments, D9res_D9treat and NoRes_D9_treat
res2 <- results( dds, contrast = c("Treatment", "D9res_D9Treat", "NoRes_D9_treat") ) 
res2

# This section will allow us to sort and export the data as a table
resOrdered2 <- res2[order(res2$padj),]
resOrderedDF_2 <- as.data.frame(resOrdered2)


# Now we're going to use the gprofiler library to assign gene names to the data. Right now, all we have is the big long ENSG ID number
resOrderedDF_v22 <- resOrderedDF_2 %>%
  rownames_to_column('ENSG_ID')

# Get the names from a gProfiler query
resOrdered_name2 <- gconvert(resOrderedDF_v22$ENSG_ID, organism = "hsapiens", target = "HGNC", filter_na = F) %>%
  dplyr::select(input, name) %>%
  dplyr::rename(ENSG_ID = input, gene_name = name) # change input to ENSG_ID

# Join names dataframe to resOrdered df by the common column of ENSG_ID
res_export2 <- inner_join(resOrdered_name2, resOrderedDF_v22, by = "ENSG_ID")

# Export the dataframe to a csv file for downstream analysis
write.csv(res_export2, 
          file="/Users/fitzsimmonscm/Documents/Projects_and_Data/Collaborations/Gottesman/RNASeq_Bergel/2019.03.16_Rhomedepsin_RNASeq_R_D9_vs_NR_D9.csv")
...
```{r}
# Here, we are asking to compare 2 treatments
res3 <- results( dds, contrast = c("Treatment", "NoRes_D9_treat", "control") ) 
res3

# This section will allow us to sort and export the data as a table
resOrdered3 <- res3[order(res3$padj),]
resOrderedDF_3 <- as.data.frame(resOrdered3)


# Now we're going to use the gprofiler library to assign gene names to the data. Right now, all we have is the big long ENSG ID number
resOrderedDF_v33 <- resOrderedDF_3 %>%
  rownames_to_column('ENSG_ID')

# Get the names from a gProfiler query
resOrdered_name3 <- gconvert(resOrderedDF_v33$ENSG_ID, organism = "hsapiens", target = "HGNC", filter_na = F) %>%
  dplyr::select(input, name) %>%
  dplyr::rename(ENSG_ID = input, gene_name = name) # change input to ENSG_ID

# Join names dataframe to resOrdered df by the common column of ENSG_ID
res_export3 <- inner_join(resOrdered_name3, resOrderedDF_v33, by = "ENSG_ID")

# Export the dataframe to a csv file for downstream analysis
write.csv(res_export3, 
          file="/Users/fitzsimmonscm/Documents/Projects_and_Data/Collaborations/Gottesman/RNASeq_Bergel/2019.03.16_Rhomedepsin_RNASeq_NR_D9_vs_NR_V_D9.csv")
```
```{r}
# Here, we are asking to compare 2 treatments
res4 <- results( dds, contrast = c("Treatment", "D9res_D9Treat", "D9_vehicle") ) 
res4

# This section will allow us to sort and export the data as a table
resOrdered4 <- res4[order(res4$padj),]
resOrderedDF_4 <- as.data.frame(resOrdered4)


# Now we're going to use the gprofiler library to assign gene names to the data. Right now, all we have is the big long ENSG ID number
resOrderedDF_v44 <- resOrderedDF_4 %>%
  rownames_to_column('ENSG_ID')

# Get the names from a gProfiler query
resOrdered_name4 <- gconvert(resOrderedDF_v44$ENSG_ID, organism = "hsapiens", target = "HGNC", filter_na = F) %>%
  dplyr::select(input, name) %>%
  dplyr::rename(ENSG_ID = input, gene_name = name) # change input to ENSG_ID

# Join names dataframe to resOrdered df by the common column of ENSG_ID
res_export4 <- inner_join(resOrdered_name4, resOrderedDF_v44, by = "ENSG_ID")

# Export the dataframe to a csv file for downstream analysis
write.csv(res_export4, 
          file="/Users/fitzsimmonscm/Documents/Projects_and_Data/Collaborations/Gottesman/RNASeq_Bergel/2019.03.16_Rhomedepsin_RNASeq_R_D9_vs_R_V_D9.csv")
```
